...
import com.slibs.slibs.infrastructure.parsers.ParserPypi;
...
class ParserPypiTests {
    ...
    void whenNormalSearch_thenSearch_returnsLibraries() {
        ...
        when(mockClient.getLibsDto("pypi", 0, size))
            .thenReturn(libsDto);
        when(mockLibraryMapper.toLibraryList(libsDto))
            .thenReturn(libs);
        
        var parser = new ParserPypi(mockClient, mockLibraryMapper);
    ...
    void whenEmptySearch_thenSearch_returnsEmptyList() {
        ...
        when(mockClient.getLibsDto("pypi", 0, 0)).thenReturn(libsDto);
    ...
    void whenLargeSearch_thenSearch_returnsLibraries() {
        ...
        when(mockClient.getLibsDto("pypi", 0, size)).thenReturn(libsDto);
        when(mockLibraryMapper.toLibraryList(libsDto)).thenReturn(libs);
